...
contract('NewMine', ([alice, bob, carol, dev, minter]) => {
    beforeEach(async () => {
        this.xNew = await XNew.new({ from: alice });

        // console.log(this.xNew.transactionHash);
        // console.log("xNew:"+ this.xNew.address);  
    ...
    it('should set correct state variables', async () => {
        ...
        const xNewPerBlock = web3.utils.toWei("10", 'ether');
        const number = await web3.eth.getBlockNumber();
        // const startBlock = number + 600; // 30分钟后开启
        const startBlock = number; // 直接开挖
        const oneYearBlock = 365*24*60*20; //挖一年

        this.newMine = await NewMine.new(this.xNew.address, wnew, xNewPerBlock, startBlock, startBlock+oneYearBlock, {from: alice});
        const xNew = await this.newMine.xNew();
        const getWNew = await this.newMine.wNew();
        const getXNewPerBlock = await this.newMine.xNewPerBlock();
        ...
        assert.equal(xNew, this.xNew.address);
        assert.equal(getWNew, wnew);
        assert.equal(getXNewPerBlock.valueOf(), xNewPerBlock);
        ...
        const xNewOwner = await this.xNew.owner();
        assert.equal(xNewOwner, this.newMine.address);
    });

    it('should activate newMine', async () => {
        const number = await web3.eth.getBlockNumber();
        const endBlock = number + 2*365*24*60*20; //挖2年   
        const xNewPerBlock = web3.utils.toWei("1", 'ether');

        await this.newMine.activate(endBlock, xNewPerBlock, true);
        const getXNewPerBlock = await this.newMine.xNewPerBlock();
        const getEndBlock = await this.newMine.endBlock();
        assert.equal(getXNewPerBlock.valueOf(), xNewPerBlock);
    ...
    context('With ERC/LP token added to the field', () => {
        ...
        it('should allow emergency withdraw', async () => {
            ...
            assert.equal(pool.accXNewPerShare, 0);
        ...
        it('should give out xNews only after farming time', async () => {
            const number = await web3.eth.getBlockNumber();
            ...
            assert.equal((await this.xNew.balanceOf(bob)).valueOf(), '1000');
            await time.advanceBlockTo(number+104);
            await this.newMine.deposit(0, '0', { from: bob }); // block 105
            assert.equal((await this.xNew.balanceOf(bob)).valueOf(), '5000');
            assert.equal((await this.xNew.totalSupply()).valueOf(), '5000');
        });

        it('should not distribute xNews if no one deposit', async () => {
            const number = await web3.eth.getBlockNumber();
            const startBlock = number + 100;
            // 1000 per block farming rate starting at startBlock with bonus until block startBlock+1000
            this.newMine = await NewMine.new(this.xNew.address, this.wnew.address, '1000', startBlock, startBlock+1000, {from: alice});
            ...
            await this.wnewToken1.approve(this.newMine.address, '1000', { from: bob });
            await time.advanceBlockTo(number+99);
            assert.equal((await this.xNew.totalSupply()).valueOf(), '0');
            await time.advanceBlockTo(number+104);
            assert.equal((await this.xNew.totalSupply()).valueOf(), '0');
            ...
            await this.newMine.withdraw(0, '10', { from: bob }); // block 120
            assert.equal((await this.xNew.totalSupply()).valueOf(), '10000');
            assert.equal((await this.xNew.balanceOf(bob)).valueOf(), '10000');
            assert.equal((await this.wnewToken1.balanceOf(bob)).valueOf(), '1000');
        });

        it('should distribute xNews properly for each staker', async () => {
            const number = await web3.eth.getBlockNumber();
            const startBlock = number + 100;
            // 1000 per block farming rate starting at startBlock with bonus until block startBlock+1000
            this.newMine = await NewMine.new(this.xNew.address, this.wnew.address, '1000', startBlock, startBlock+1000, {from: alice});
        ...
        it('should give proper xNews allocation to each pool', async () => {
            const number = await web3.eth.getBlockNumber();
            const startBlock = number + 100;
            // 1000 per block farming rate starting at startBlock with bonus until block startBlock+1000
            this.newMine = await NewMine.new(this.xNew.address, this.wnew.address, '1000', startBlock, startBlock+1000, {from: alice});
